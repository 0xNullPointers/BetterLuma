...
.DESCRIPTION
    ...
    installation directory, backing up existing conflicting DLLs, opensteamtool.dll, LumaCore.dll, and LumaCorePayload.dll
    to .bak, deploying BetterLuma.toml, and generating a dynamic uninstaller script.
.PARAMETER SteamPath
...
.PARAMETER DownloadUrl
    ...
    automatically queries GitHub for the latest release and downloads Debug.zip.
#>
...
# Step 3: Download & Extract BetterLuma Release Zip
function Get-LatestBetterLumaUrl {
    param(
        [string]$Repo = "0xNullPointers/BetterLuma",
        [string]$AssetPattern = "*Debug*.zip"
    )
    ...
    $fallbackUrl = "https://github.com/$Repo/releases/latest/download/Debug.zip"
...
if (-not $DownloadUrl) {
    Write-Host ""
    Write-Host "[*] Resolving latest BetterLuma release from GitHub..." -ForegroundColor Cyan
    $DownloadUrl = Get-LatestBetterLumaUrl
}

$tempRoot = Join-Path $env:TEMP ("BetterLuma_Setup_" + [System.Guid]::NewGuid().ToString("N"))
$tempZip  = Join-Path $tempRoot "BetterLuma_Debug.zip"
...
try {
    ...
    # Explicit priority files to back up if present: opensteamtool.dll, LumaCore.dll, LumaCorePayload.dll
    $priorityFiles = @(
        'opensteamtool.dll',
        'LumaCore.dll',
        'LumaCorePayload.dll'
    ...
    foreach ($dll in $extractedDlls) {
        if (-not $targetsToScan.Contains($dll.Name)) {
