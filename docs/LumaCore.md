...
### SteamCapture & RuntimeCapture (`hooks/capture/RuntimeCapture.cpp`)
...
**Lifecycle Tracking & Startup Grace Period (`OnlineFixAppEntry`)**:
`SteamCapture` maintains a map of `OnlineFixAppEntry` structs recording active AppIDs, primary/child PIDs, `seenInPacket` flags, and `registeredAt` timestamps. A 20-second startup grace period (`CanUnregisterOnlineFixApp`) prevents periodic `CMsgClientGamesPlayed` heartbeat packets from prematurely unregistering a game while another is launching. Active route contexts and rich presence are maintained until all OnlineFix processes have exited.
...
- **`SendCallbackToPipe`**: intercepts `AppLicensesChanged` callbacks and forces `m_bReloadAll = true` so Steam fully refreshes its license state after package changes. Also dual-dispatches achievement callbacks for concurrent OnlineFix titles.
...
[pattern_fetch]
# Template placeholders: {channel}, {component}, {subdir}, {sha256}, {sha}
url_template = ""
...
Manual `-onlinefix` is intentionally separate. It uses the full online-fix route with payload injection and real game name exposure, so the log set should show `routeMode=manual-flag`, raw env `480/480/<real appid>`, GetAppID/packet rewrite proof, the manual packet patch, and a clean latest exit instead of SteamStub-only GamesPlayed hiding.
