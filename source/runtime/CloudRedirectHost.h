...
// drives its third-party client API:
...
//
// All entry points are safe no-ops unless [cloud].enabled is true and the DLL
// loaded and initialised successfully.
namespace CloudRedirectHost {

    // Load + initialise. Called once from the worker thread, after hooks are installed.
    // steamInstallPath is the Steam root directory.
    void Initialize(const char* steamInstallPath);

    // Re-push the current unlocked-app set to CloudRedirect. Called after a Lua
    // reload so the redirected set tracks addappid() changes.
    void SyncAppSet();
    ...
    // Teardown. Called from DLL_PROCESS_DETACH or Orchestrator::Detach.
    void Shutdown();
